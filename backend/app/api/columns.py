...
from app.models.models import BoardColumn, User
...
def create_column(payload: ColumnCreate, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
...
def patch_column(column_id: int, payload: ColumnPatch, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ensure_admin(current_user)
