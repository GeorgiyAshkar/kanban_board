...
## Kaiten-inspired capabilities added

После анализа ключевых возможностей Kaiten приложение усилено в направлении гибкой Kanban-платформы:

- **Дорожки (swimlanes)** на доске: группировка по приоритету, исполнителю, проекту или блокировкам для визуализации параллельных потоков работ.
- **WIP/SLA flow-control** на колонках: лимиты незавершенной работы и предупреждения о просрочке SLA.
- **Проекты, классы обслуживания, типы работ и блокировки карточек**: проект можно назначить при создании или редактировании задачи; у задачи можно указать класс обслуживания, тип работ, explicit policy, статус блокировки, причину и ID карточки-блокера; срочные/фиксированные/заблокированные карточки подсвечиваются на доске и попадают в аналитику.
- **Flow-отчеты**: Lead Time, Cycle Time, WIP, velocity, открытые блокировки и базовая оценка flow efficiency.
- **Классы обслуживания и типы работ**: карточки поддерживают `Standard`, `Fixed date`, `Expedite`, `Intangible`, типы работ (`feature`, `bug`, `support`, `ops`, `research`) и явные политики карточек; это приближает доску к Kaiten-практикам service classes, explicit policies и анализу потока.

Архитектурно backend сохраняет микросервисные границы внутри приложения: доменные роутеры `tasks`, `analytics`, `notifications`, `backup`, `history` и сервисные модули могут независимо выноситься в отдельные контейнеры при масштабировании. В Docker Compose уже разделены frontend, основной backend и отдельный PostgreSQL-профиль.
...
- Для колонок поддерживаются `wip_limit` и `sla_hours` (настройка flow-control и SLA-контроля).
...
- Экспорт/бэкап: `GET /backup/export.json`, `GET /backup/export.csv`, `GET /backup/archive`.
- Импорт/восстановление: `POST /backup/import` с режимами `merge` и `replace_all` + `dry_run`.
...
- В правой панели после редактирования используются вкладки: Теги / Чек-лист / Комментарии / История; панель прокручивается при большом объеме полей и активности.
...
- В `Настройках` добавлено управление `WIP`-лимитом и `SLA` (часы) для каждой колонки.
...
- Доска умеет строить дорожки по классу обслуживания и типу работ, а отчеты показывают распределение активного WIP по этим измерениям.
