...
- **Альтернативные представления задач**: кроме канбан-доски доступны табличный вид для аудита карточек, календарный вид по дедлайнам/датам возврата и таймлайн для планирования работ во времени.
- **Контроль незавершенной работы и сроков** на колонках: лимиты незавершенной работы и предупреждения о просрочке SLA.
- **Проекты, классы обслуживания, типы работ и блокировки карточек**: проект можно назначить при создании или редактировании задачи; у задачи можно указать класс обслуживания, тип работ, явную политику, статус блокировки, причину и ID карточки-блокера; срочные/фиксированные/заблокированные карточки подсвечиваются на доске и попадают в аналитику.
- **Отчеты по потоку**: время выполнения, время в работе, незавершенная работа, скорость завершения, открытые блокировки и базовая оценка эффективности потока.
- **Классы обслуживания и типы работ**: карточки поддерживают `Стандартный`, `Фиксированная дата`, `Срочный`, `Улучшение`, типы работ (`feature`, `bug`, `support`, `ops`, `research`) и явные политики карточек; это приближает доску к практикам Kaiten: классам обслуживания, явным политикам и анализу потока.

Архитектурно backend сохраняет микросервисные границы внутри приложения: доменные роутеры `tasks`, `analytics`, `notifications`, `backup`, `history` и сервисный модуль `automation` могут независимо выноситься в отдельные контейнеры при масштабировании. В Docker Compose уже разделены frontend, основной backend и отдельный PostgreSQL-профиль.
...
- Доска поддерживает переключение между канбан-представлением, таблицей, календарем и таймлайном для планирования дедлайнов и просмотра задач с разной детализацией.
...
- Доработать календарь/таймлайн до полноценной месячной сетки и drag-and-drop переноса дат.
