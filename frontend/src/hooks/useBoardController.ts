...
export const useBoardController = () => {
  ...
  return {
    ...
    actions: {
      ...
      backupExportUrl: (kind: 'json' | 'csv' | 'archive') => buildBackupDownloadUrl(kind),
      importBackup: async (backupPayload: Parameters<typeof importBackup>[0], mode: 'merge' | 'replace_all') => {
        const result = await importBackup(backupPayload, { dryRun: false, mode });
        await refreshBoardData();
        return result;
      },
      dryRunBackupImport: async (backupPayload: Parameters<typeof importBackup>[0], mode: 'merge' | 'replace_all') =>
        importBackup(backupPayload, { dryRun: true, mode }),
    },
  };
};
